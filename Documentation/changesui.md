...
## 4. Color Palette & Design Tokens (Extracted from Ananya-Frontend)

> [!NOTE]
> The design tokens below have been extracted directly from the **Ananya Frontend** design system (`Ananya frontend/src/index.css` and core components).

| Token Name | Custom Hex / Variable | UI Component Mapping | Notes / Guidelines |
|---|---|---|---|
| `brand-primary` | `#4f7a65` (`var(--accent)`) | Primary CTAs, active agent orb glow, active nav tabs & main brand accents | Sage Emerald Green |
| `brand-accent` | `rgba(79, 122, 101, 0.12)` | Secondary highlights, agent badges, active chip fills, selection background | Light Sage 12% Opacity Tint |
| `status-live` | `#c26a5a` / `#4f7a65` | Live call recording indicator, pulse ring, status badges | Terracotta Red / Sage Green Pulse |
| `user-speaking` | `#4f7a65` / `#efede8` | User speaking orb state, user transcript badge (`var(--muted)`) | Sage Green Orb & Light Gray Bubble |
| `surface-bg` | `#f6f5f2` (`var(--background)`) | Fullscreen modal background, page backdrop | Warm Ivory Off-White |
| `surface-card` | `#ffffff` (`var(--card)`) / `#fcfbf8` (`var(--surface)`) | Modal inner panels, transcript container, input fields, elevated surfaces | Pure White & Soft Parchment |
| `border-subtle` | `#e6e4df` (`var(--border)`) | Modal borders, card outlines, divider lines, custom scrollbar thumb | Soft Warm Gray |
| `text-primary` | `#202124` (`var(--foreground)`) | Headings, primary button text, main dialogue text, dark CTA container | High-Contrast Deep Charcoal |
| `text-muted` | `#6b6b6b` (`var(--muted-foreground)`) | Call timers, status subtext labels, placeholder text, secondary metadata | Muted Slate Gray |
| `font-sans` | `'Inter', 'Geist', sans-serif` | Main interface typography, headings & body text | Clean Modern Sans-Serif |
| `font-mono` | `ui-monospace, monospace` | Timers, LIVE badges, technical metadata, latency indicators | Precision Monospace |
